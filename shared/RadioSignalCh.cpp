...
// CRadioSignalCh::CRadioSignalCh(  TXParams* MR, float HAnt, float DN, float H2_DOP, float F0, float hrgab, float hbur )
// {
// //	ASSERT(F0);
//    //1
// // 	m_F0 = F0;       -------------------------------
...
// 	m_RadioP = 0.f;
// 
// 	m_HCherry = hrgab;    // высота радиоантенны 
// 	m_HPom  = hbur;     // высота радиоантенны 
// 
// }

// CRadioSignalCh::CRadioSignalCh( TXParams* MR, float HAnt, float DN, float H2_DOP, float hrgab, float hbur )
// {
//    //2    ------------------------
...
void CRadioSignalCh::Store00( float DN, float H2_DOP, TTtxCherry* TTX /*, float hrgab, float hbur*/ )
{
   m_HCherry = TTX -> HAnt;
	m_H2_Dop = H2_DOP;   // пороговое H2
	m_DN   = DN;
...
void CRadioSignalCh::Store0( TXParams* MR, TTtxCherry* TTX )
{
	X = MR->X;
	Y = MR->Y;
	Z = MR->Z;
   ...
   m_CherryTipNK = 0;  // Тип обнаруженного канала Ч
	m_CherryNK = 0;     // N обнаруженного канала Ч
   m_CherryId = 0;     // Id прибора
	m_CherryPower = 0;  // Пришедшая мощность на обнаруженном канале Ч
...
int CRadioSignalCh::AddSignal( CRWave *ro )
{  // вычисление мощности принятого сигнала, возвращает 1 - сигнал, надо записывать  P D
   ...
   // высота антенны     км
	...
	float h = ( m_HCherry - ro->HAnt ) * 0.001f;     // / 1000.f; км
//    float xx = x * x;
//    float yy = y * y;
   float xy = x * x + y * y;
...
// 	float dist3 = Distance3( ro->X, ro->Y, m_HRgab, X, Y, (float)fabs(Z) );  // км
   ...
   {  // замена сигнала большим
      ...
      m_CherryTipNK = ro->CherryTipNK;     // Id прибора                               
	   m_CherryNK    = ro->CherryNK;     // N обнаруженного канала Ч                 
      m_CherryId    = ro->CherryId;  // Тип обнаруженного канала Ч                  
	   m_CherryPower = ro->CherryPower;  // Пришедшая мощность на обнаруженном канале Ч 
...
void CRadioSignalCh::AddPomeha( CRWave *ro )
{  // добавить мощность помехи от ro
   // --- км
	float x = ro->X - X/1000.f;
	float y = ro->Y - Y/1000.f;
	float h = ( ro->HAnt - m_HCherry ) * 0.001f;     // / 1000.f; км
   float xx = x * x;
   float yy = y * y;
   float xy = xx + yy;
	float dist2 = (float)(sqrt( xy ));  // км
...
// 	float dist3= Distance3( ro->X, ro->Y, m_HBur, X, Y, (float)fabs(Z) );
   ...
   if ( m_RGBPower <= m_RadioPnew ) 
   {
      ...
      m_CherryTipNK = -1;  // Тип обнаруженного канала Ч
	   m_CherryNK    = -1;     // N обнаруженного канала Ч
      m_CherryId    = -1;     // Id прибора
	   m_CherryPower = -1;  // Пришедшая мощность на обнаруженном канале Ч
