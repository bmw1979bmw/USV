...
class CRadioSignalCh : public TXParams // TXParams объекта от точки рождения волны:
{
public:
	
public:
	
// 	CRadioSignalCh( TXParams* MR, float m_HAnt, float DN, float H2_DOP, float F0, float hrgab, float hbur );
// 	CRadioSignalCh( TXParams* MR, float HAnt, float DN, float H2_DOP, float hrgab, float hbur );
	CRadioSignalCh();
//    CRadioSignalCh( float DN, float H2_DOP, float hrgab, float hbur );    // !!!
	
	int   AddSignal( CRWave *ro );  // вычисление мощности принятого сигнала
	void  AddPomeha( CRWave *ro );  // добавить мощность помехи от ro
	void Store0( TXParams* MR, TTtxCherry* TTX );
	void Store00( float DN, float H2_DOP, TTtxCherry* TTX  /*, float hrgab, float hbur*/ );
	
inline float DMax( float h )      //   вычисление максимальной дистанции радиопередачи
{  // вычисление максимальной дистанции радиопередачи
	float dsv = 4.12f * ( (float)sqrt( (float)fabs(m_HCherry) ) + (float)sqrt( (float)fabs(h) ) ); // дистанция связи
	return dsv;     //7.f;
}      
...
// 	float m_HAnt;     // высота радиоантенны приемника
	...
	float m_HCherry;  // высота радиоантенны 
   ...
   int   m_CherryTipNK;  // Тип обнаруженного канала Ч
	int   m_CherryNK;     // N обнаруженного канала Ч
   int   m_CherryId;     // Id прибора
	float m_CherryPower;  // Пришедшая мощность на обнаруженном канале Ч
};
