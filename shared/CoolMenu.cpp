...
//
// Changed by Long 04.09.03 11:43
//              // 23.05.2013
#include "StdAfx.h"
#include "CoolMenu.h"
...
struct CCoolMenuItemData {
	...
	CCoolMenuItemData()				{ magicNum = MYITEMID; }
	BOOL IsMyItemData()		{ 
      __try {
         return magicNum == MYITEMID; 
      } __except(1) {
         return 0;
      };
//      return 0;
...
BOOL CCoolMenuManager::LoadToolbar(UINT nIDToolbar, COLORREF clrTransparent)
{
   ...
   struct TOOLBARDATA {
	...
	// load bitmap
	HBITMAP hbmToolbar = 
      ::LoadBitmap(AfxGetResourceHandle(), (LPCTSTR)nIDToolbar);
      //Y- PLLoadSysColorBitmap(nIDToolbar);
	...
	if (m_szBitmap.cx==0) {
		...
		VERIFY(m_ilButtons.Create(sz.cx, sz.cy, ILC_COLORDDB|ILC_MASK, 0, 0));
	...
	m_ilButtons.Add(&bmToolbar, clrTransparent );
	for (int i = 0; i < ptbd->wItemCount; i++) {
		UINT nID = ptbd->items[i];
		if (nID > 0) {
			if (GetButtonIndex((WORD)nID) >= 0) {
				TRACE(_T("*** Duplicate button ID %d ignored\n"), nID);
			} else
				m_mapIDtoImage.SetAt((WORD)nID, (void*)iNextImage);
         iNextImage++;
		}
	}
...
//       HKL hkl; // RUSSIAN_CHARSET
// 		SystemParametersInfo(SPI_GETDEFAULTINPUTLANG, sizeof(hkl), &hkl, 0);
      info.lfMenuFont.lfCharSet = RUSSIAN_CHARSET;
//       info.lfMenuFont.lfClipPrecision = 0x40;
//       lstrcpy( info.lfMenuFont.lfFaceName,"Arial" /*"MS Sans Serif"*/);
...
{
	ASSERT(lpms);
	CCoolMenuItemData* pmd = (CCoolMenuItemData*)lpms->itemData;
	ASSERT(pmd);
	...
	if (pmd->fType & MFT_SEPARATOR) {
		...
		lpms->itemHeight= max(
         max(GetSystemMetrics(SM_CYMENU), rcText.Height()),
         m_szButton.cy
         )
         ;
...
{
	ASSERT(lpds);
	CCoolMenuItemData* pmd = (CCoolMenuItemData*)lpds->itemData;
	ASSERT(pmd);
	...
	if (pmd->fType & MFT_SEPARATOR) {
      ...
      rc.right-=3;
		...
		if (iButton >= 0) {
			...
			CCoolMenuItemInfo info;
			info.fMask = MIIM_CHECKMARKS;
			if ( GetMenuItemInfo((HMENU)lpds->hwndItem,
...
void CCoolMenuManager::LoadAccel(HACCEL hAccel)
{
	...
	if (hAccel && (nAccel = CopyAcceleratorTable(hAccel, NULL, 0)) > 0) {
		...
		for (int i=0; i<nAccel; i++) {
			...
			ac.cmd = WORD( pAccel ? &ac - pAccel : 0 ); // ac.cmd = offset of prev, or 0
		}
	}
}
	...
	UINT nIndex, BOOL bSysMenu, BOOL bShowButtons)
{
	...
	for (UINT i = 0; i < nItem; i++) {	// loop over each item in menu
		...
		if ( !::GetMenuItemInfo(*pMenu, i, TRUE, &info) ) continue;
		CCoolMenuItemData* pmd = (CCoolMenuItemData*)info.dwItemData;
		...
		if (bShowButtons) {
			...
			if (!(info.fType & MFT_OWNERDRAW)) {
				...
				if (!pmd) {									// if no item data:
					pmd = new CCoolMenuItemData;				//   create one
					ASSERT(pmd);							//   (I hope)
...
HBITMAP CCoolMenuManager::GetMFCDotBitmap()
{
	...
	CCoolMenuItemInfo info;
	info.fMask = MIIM_CHECKMARKS;
	GetMenuItemInfo(menu, 0, MF_BYPOSITION, &info);  // no good...  if not?
